	...
	public static List<List<String>> generateReport() {
		...
		try {
			...
			if (!con.isClosed()) {
				...
				//ResultSet res = stmt.executeQuery("SELECT * FROM visitors NATURAL JOIN visits NATURAL JOIN visitorlocations ORDER BY Visiting_Day");
				ResultSet res = stmt.executeQuery("SELECT * FROM visitors LEFT JOIN visits ON visitors.VisitorID = visits.VisitorID LEFT JOIN visitorlocations on visitors.VisitorID = visitorlocations.VisitorID ORDER BY Visiting_Day");
