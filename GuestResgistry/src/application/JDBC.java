...
public class JDBC {
	...
	public static void insertIntoDB(Visitor visitor) {
		...
		String url = "jdbc:mysql://localhost:3306/test";// will replace
														// 'localhost' with
														// proper ip address
														// when time comes
		String user = "root";
		...
		try {
			...
			if (!con.isClosed()) {
				...
				/***********************************************************************
				 * The series of if statements below modifies the input values
				 * as needed to prevent the database from exploding.
				 *********************************************************************/

				/*
				 * This if statement is basically just a placeholder for when we
				 * add in the hasVisited functionality for the form. It adds in
				 * a 'true' value by default at the moment.
				 */
				...
				/*
				 * This if-else statement checks to see if the value from the
				 * 'heard' drop down menu is null. It sets the value to
				 * "No Response" if it is, and leaves it alone otherwise.
				 */

				String hear = "";
				if (!(visitor.getHeard() == "Billboard") || !(visitor.getHeard() == "Interstate Sign")
						|| !(visitor.getHeard() == "Other")) {
					hear = "No Response";
				} else {
				...
				/*
				 * This if-else statement is the same as above, but for the
				 * other drop down menu.
				 */

				String reason = "";
				if (!(visitor.getReason() == "Business") || !(visitor.getReason() == "Convention")
						|| !(visitor.getReason() == "Pleasure") || !(visitor.getReason() == "Other")) {
					reason = "No Response";
				} else {
				...
				/*
				 * This if-elseif-else block essentially does the same as the
				 * above two; it sets the proper value for the radio buttons as
				 * needed.
				 */
				...
				/*
				 * This series of stmt.execute methods insert the data into the
				 * database. It's kind of ugly, but it works.
				 */
				...
				stmt.execute(
						"INSERT INTO visits (VisitorID, Party, Heard, Hotel, Destination, RepeatVisit, TravelingFor, Visiting_Day) VALUES"
								+ "(" + visitor.getId() + ", '" + visitor.getParty() + "' ," + "'" + hear + "'" + ", '"
								+ hotel + "', " + "'" + visitor.getDestination() + "', " + "'" + v + "'" + ", '"
								+ reason + "', " + "'" + getCurrentTimeStamp() + "'" + ")");
			} // end if
		} // end try
